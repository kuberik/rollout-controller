...
type ReleaseConstraintReconciler struct {
	client.Client
	Scheme *runtime.Scheme
}

// +kubebuilder:rbac:groups=kuberik.com,resources=releaseconstraints,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=kuberik.com,resources=releaseconstraints/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=kuberik.com,resources=releaseconstraints/finalizers,verbs=update

// Reconcile is part of the main kubernetes reconciliation loop which aims to
// move the current state of the cluster closer to the desired state.
// TODO(user): Modify the Reconcile function to compare the state specified by
// the ReleaseConstraint object against the actual cluster state, and then
...
func (r *ReleaseConstraintReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&kuberikcomv1alpha1.ReleaseConstraint{}).
		Named("releaseconstraint").
		Complete(r)
}
